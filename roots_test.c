...
void test_find_roots_1() {
  ...
  assert(fabs(res[0] - 2) < 10E-7 && fabs(res[1] + 1) < 10E-7 && fabs(res[2] - 1) < 10E-7);
}
...
void test_find_roots_4() {
  ...
  double c = -1e-7;
  double *res = find_roots(a, b, c);
  assert(fabs(res[0] - 2) < 10E-7 && fabs(res[1] + 3e-4) < 10E-5 && fabs(res[2] - 3e-4) < 10E-5);
}
void test_find_roots_5() {
  ...
  double c = -1;
  double *res = find_roots(a, b, c);
  assert(fabs(res[0] - 2) < 10E-7 && fabs(res[1] + 1e-10) < 10E-11 && fabs(res[2] - 1e+10) < 10E-11);
}
